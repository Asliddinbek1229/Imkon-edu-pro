...
def course_catalog_text(courses: list, page: int, total: int) -> str:
    ...
    for index, course in enumerate(courses, start=1):
        ...
        lines.append(
            f"{index}. {status} <b>{html.quote(course['name'])}</b> — {format_price(course['price'])}"
        )

    lines.extend(
        [
            "",
            "Batafsil ma'lumot olish uchun pastdagi raqamlardan birini tanlang.",
        ]
    )
    return "\n".join(lines)
...
def course_detail_text(course) -> str:
    includes = course["includes"] or "Video darslar, chizmalar, testlar va DTM tahlili"
    target_exam = course["target_exam"] or "DTM, Milliy Sertifikat, Attestatsiya"
    duration = course["duration"] or "Kurs davomiyligi admin tomonidan belgilanadi"

    return (
        f"📘 <b>{html.quote(course['name'])}</b>\n"
        f"👨‍🏫 {html.quote(course['author'])}\n\n"
        f"💰 <b>Narx:</b> {format_price(course['price'])}\n"
        f"📊 <b>Tarkib:</b> {course['video_count']} ta video dars\n"
        f"⏱ <b>Davomiylik:</b> {html.quote(duration)}\n"
        f"📝 <b>Qo'shimcha:</b> {html.quote(includes)}\n"
        f"🎯 <b>Imtihonlar uchun:</b> {html.quote(target_exam)}\n"
        f"♾️ <b>Kirish:</b> {html.quote(course['access_type'])}\n\n"
        "<b>TAVSIF:</b>\n"
        f"{html.quote(course['description'])}"
    )


def course_detail_keyboard(course_id: int, page: int) -> InlineKeyboardMarkup:
...
async def render_course_detail(call: types.CallbackQuery, course_id: int, page: int) -> None:
    ...
    markup = course_detail_keyboard(course_id=course_id, page=page)
...
async def catalog_buy(call: types.CallbackQuery, callback_data: CatalogActionCallback):
